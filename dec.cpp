...
void MainProcess()
{
	...
	while ((fgets(StrLine, LINE_SIZE, fp_in))!=NULL)
	{
		...
		/*if (Count_Out == 10000)
		{
		printf("---------%d line processed CurrentLineNumber = %d---------\n", Count_Out,CurrentLineNumber);
		Count_Out = 0;
		}
		*/
		//printf("------------each line before while-------------\n");
		while (StrLine[StrPos] != '\0')
		{

			//printf("StrLine[StrPos] = %c  StrPos=%d\n", StrLine[StrPos],StrPos);
			//printf("---------CurrentLineNumber= %d\n-----------", CurrentLineNumber);
			Match = 0;
			if (((((int)StrLine[StrPos]) & 0xFF) > 128) && ((((int)StrLine[StrPos]) & 0xFF)<255)) //有匹配 第一种匹配情况 匹配数 <127
			{
				...
				//printf("有匹配 第一种匹配情况 匹配数 <127 StrLine[StrPos] = %c  StrPos=%d\n", StrLine[StrPos], StrPos);
			...
			{
				//printf("else StrLine[StrPos] = %d  StrPos=%d\n", (((int)StrLine[StrPos]) & 0xFF), StrPos);
				...
				else  //StrLine[StrPos] < 127   没有匹配，则直接将StrLine的数据拷贝到AfterTrans中
				{
					...
					/*
					if (((StrLine[StrPos] == ' '))&&(StrLine[StrPos] != '\0'))
					{
					AfterTrans[TransPos++] = ' ';
					StrPos++;
					}
					*/
					...
					/*
					if ((PreviousLine[PrePos] == ' ') && (PreviousLine[PrePos] != '\0'))
					{
					PrePos++;
					}
					*/
		...
		/*
		int pos = 0;
		while ((AfterTrans[TransPos - pos] == '\n') || (AfterTrans[TransPos - pos] == '\0'))
		{
		pos++;
		}
		AfterTrans[TransPos - pos + 1] = '\n';
		AfterTrans[TransPos - pos + 0] = '\0';
		*/
